...
server.registerTool(
  ...
  {
    description:
      "List all available components in the component library. Returns component names and the stories directory path.",
    inputSchema: {},
  },
  async () => {
...
server.registerTool(
  ...
  {
    description:
      "Get detailed documentation for a specific component: props, variants, usage examples, and design constraints.",
    inputSchema: {
      componentName: z
        .string()
  ...
  async ({ componentName }) => {
    ...
    if (!fs.existsSync(storyPath)) {
      const available = getStoryFiles().map((f) =>
        f.replace(".stories.tsx", "")
      );
      return {
        content: [
          {
...
server.registerTool(
  ...
  {
    description:
      "Search component names by keyword. Useful for checking if a component already exists before building a new one.",
    inputSchema: {
