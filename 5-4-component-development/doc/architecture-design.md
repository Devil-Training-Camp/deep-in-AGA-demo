...
本文档承接两份上游产出：`requirements.md` 定义了六大功能与边界，`tech-selection.md` 给出了带约束推导的技术选型。本轮在架构设计阶段又敲定了一组运行时约束，先说清楚。
...
选型文档里的核心结论在架构阶段全部直接沿用：**pgvector**（决策点 2）、**知识库字段过滤隔离**（决策点 3）、**query rewriting**（决策点 5B）、**PG 两表存历史**（决策点 6）、**LLM/Embedding 走云 API + 1024 维**（决策点 7）。文档解析用 `pdf-parse`(MIT) + `mammoth`(BSD) + Node 内置 `fs`，纯 JS、无系统依赖，也不牵涉任何 copyleft 许可问题。
...
- **决策**：Next.js API Routes 以 `next start` 跑在单个长驻 Node 进程上（VM / 容器），Route Handler 显式声明 `runtime = "nodejs"`。
...
- **理由**：复用 Supabase 的 PG，**守住"不引入 Redis"**；任务持久化，进程重启不丢任务。
...
- **理由**：不自担密码哈希、找回流程、token 签发/轮转的安全责任；Supabase 直接给出 SSR 场景的 cookie + session 刷新方案，单实例落地最省事。数据库本就用同一个 Supabase 项目（Auth 与 PG 一体），认证走 Supabase 不额外引入服务。
...
- **理由**：纯 JS、无系统依赖，契合"减少运维"；`pdf-parse`(MIT) 与 `mammoth`(BSD) 均为宽松许可，不牵涉 copyleft。
...
用 **Drizzle ORM**。Drizzle 原生支持 pgvector 的 `vector` 类型，向量列直接建模、检索算子内置，不需要额外的类型 workaround：

- schema 里用 `pgvector` 列类型声明 `embedding`（`vector({ dimensions: 1024 })`）；
- 相似度检索用 Drizzle 内置的 `cosineDistance`（对应 `<=>` 操作符）配 `orderBy` + `limit`，无需手写裸 SQL 序列化向量；
- 扩展启用（`CREATE EXTENSION vector`）与建表、HNSW 索引一起写进 Drizzle migration（`drizzle-kit generate` / `migrate`）。
...
### 2. 供应商抽象层（呼应选型「LLM/Embedding 厂商」待评审项）
...
3. **具体 LLM/Embedding 厂商**：延续选型「LLM/Embedding 厂商」待评审项，受公司内部 API 渠道与合规影响。
4. **在 Supabase 项目上启用 `vector` 扩展**：pgvector 落地的前置动作。Supabase 默认内置 pgvector，在 Dashboard 的 Database → Extensions 里开启 `vector` 即可（或迁移里 `CREATE EXTENSION`）。
...
评审时建议优先看三处：**三个需求微调点**（扫描件异步拦截、知识库全员共享、隔离退到历史层）是否符合产品意图；**分块策略**（决策 11）作为返工成本最高的决策是否需要在首版就投入更多；以及**待确认项 4**（在 Supabase 上启用 pgvector）作为整个向量方案的前置地基，尽早开启确认。
