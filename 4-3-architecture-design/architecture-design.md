...
- **决策**：邮箱 + 密码自建账号体系，密码用 `bcrypt`/`argon2` 哈希；登录态用 **JWT**（无状态）。
- **理由**：不依赖公司 SSO，身份自主；JWT 无状态，单实例下最简单。
- **代价**：自己承担密码安全、找回流程、首个管理员种子等责任。
- **落地要点**：JWT 签名密钥进环境变量；设合理过期时间；`role`（`user`/`admin`）放进 JWT claim；首个管理员靠 seed 脚本或环境变量指定，不经注册流程产生。**注册开放度（限公司邮箱域名 / 邀请制）见待确认项。**
...
约定：除登录/注册外，所有接口需 JWT 鉴权（`Authorization: Bearer <token>`）；错误响应统一 `{ "error": { "code": string, "message": string } }`。
