...
- **前端**：React + TypeScript。
- **数据库**：复用已有 PostgreSQL 14（Railway 托管），启用 `pgvector` 扩展。**不引入 Redis 或任何新的独立服务。**
...
目录结构见 `architecture-design.md:388`。各模块职责与不可越界的红线：
...
数据模型与接口契约见 `architecture-design.md:123`（DDL）、`architecture-design.md:246`（API）。
...
- **密钥只存服务端环境变量**：JWT 签名密钥、云 API 密钥、PG 连接串绝不硬编码、绝不进前端 bundle（禁 `NEXT_PUBLIC_` 前缀）、不进日志/错误响应。
...
注册开放度、首个管理员种子方式、具体 LLM/Embedding 厂商、Railway PG 是否支持 `CREATE EXTENSION vector`（向量方案前置地基，尽早实测）、管理员可见性告知的落地位置。详见 `architecture-design.md:439`。
