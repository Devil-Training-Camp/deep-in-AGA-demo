...
interface SceneAudio {
  path: string;            // audio/scene-XXX.mp3
  durationSec: number;     // ffprobe 量得,渲染时长的真相
...
用 `messages.parse()` 配合 `zodOutputFormat`,强制模型按 `ScriptSchema` 返回。结构化输出在 API 层校验,`parsed_output` 为 `null` 即视为失败、重试,正好实现 requirements 里的"schema 校验 + 重试":
...
if (!res.parsed_output) throw new RetryableError("schema 不合法");
const script = res.parsed_output;     // 已是合法的 { scenes: [...] }
```

注意 Opus 4.8/Sonnet 4.6 不再支持 `temperature`/`budget_tokens`,用 `thinking: {type:"adaptive"}` 控制思考。参考 [结构化输出](https://platform.claude.com/docs/en/build-with-claude/structured-outputs.md)、[模型选择](https://platform.claude.com/docs/en/about-claude/models/overview.md)。
